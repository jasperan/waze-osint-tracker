...
>
> ```bash
> curl -fsSL https://raw.githubusercontent.com/jasperan/waze-osint-tracker/main/install.sh | bash
> ```
>
...
> ```bash
> PROJECT_DIR=/opt/myapp curl -fsSL https://raw.githubusercontent.com/jasperan/waze-osint-tracker/main/install.sh | bash
> ```
>
> Or install manually:
> ```bash
> git clone https://github.com/jasperan/waze-osint-tracker.git
...
# Run (connects to Flask API on localhost:5000)
./waze-tui

# Run against a custom API URL
./waze-tui --api http://your-host:5000
```
...
| `1`-`5` | Jump to screen |
| `?` / `F1` | Help overlay |
| `Tab` | Toggle layout (dashboard) or cycle panels (investigation) |
| `c` `v` `d` `l` `n` | Switch investigation bottom view (trips, privacy, intel, location, network) |
| `/` | Filter users or events |
| `q` | Quit (from splash) or navigate back |
| `Ctrl+C` | Force quit |
